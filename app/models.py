...
from sqlalchemy import Integer, String, DateTime, Boolean, ARRAY
...
class Post(Base):
    ...
    def __repr__(self):
        return "Post<(user_id='%s', images='%s', title='%s', desc='%s', created_date='%s', last_modified_date='%s', status='%s')>" % (
            self.user_id,
            self.images,
            self.title,
            self.desc,
